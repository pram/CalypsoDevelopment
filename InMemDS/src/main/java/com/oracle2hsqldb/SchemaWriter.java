...
public class SchemaWriter {
    ...
    public String write(Table table, String createTableString) {
        if (!viewsAsTables && table instanceof View && configuration.dialect().supportsViews()) {
            return createView((View) table);
        } else {
            return createTable(table, createTableString);
        }
    }

    private String createTable(Table table, String createTableString) {
        String tableName = table.name();
        tableName = quotifyPositionName(tableName);
        StringBuffer result = new StringBuffer(createTableString).append(tableName).append(" (");
        for (Iterator columns = table.columns().iterator(); columns.hasNext();) {
            Column column = (Column) columns.next();
            result.append(write(column));
    ...
    private String quotifyPositionName(String objectName) {
        if (objectName.equalsIgnoreCase("position")) {
    ...
    public String write(Column column) {
        ...
        columnName = quotifyPositionName(columnName);
        ...
        if (column.defaultValue() != null && !column.defaultValue().getValue().trim().equals("0")) {
    ...
    private void appendIdentityColumnString(StringBuffer result) {
        result.append(" ").append(configuration.dialect().getIdentityColumnString().toUpperCase());
    }
