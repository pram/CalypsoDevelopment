...
public class jdbcConnection implements Connection {
    ...
    private static HashMap<String, String> intercepts = new HashMap<String, String>();

    public static void addSQLIntercept(String replaceThis, String withThis) {
        intercepts.put(replaceThis, withThis);
    }

    private static String interceptSQL(String sqlString){
        if (sqlString != null){
            for (String replaceThis : intercepts.keySet()){
                String withThis = intercepts.get(replaceThis);
                sqlString = sqlString.replaceAll(replaceThis, withThis);
            }
        }
        return sqlString;
    }
...
// ---------------------------- Common Attributes --------------------------
    ...
    public static HashMap<String, String> getIntercepts() {
        return intercepts;
    }

    public static void setIntercepts(HashMap<String, String> intercepts) {
        jdbcConnection.intercepts = intercepts;
    }
...
// ----------------------------------- JDBC 1 -------------------------------
    ...
    public synchronized String nativeSQL(String sql)
        ...
        // TESTME: First kick at the FIXME cat done.  Now lots of inmemds
        // and refinment
        checkClosed();
